...
info_data <- read.csv("neil/deseq2_all_samples/HFD_ff_v_HFD_ko_labels.csv",header=T,row.names=1)

info_data$log2FoldChange <- info_data$log2FoldChange * -1

norm_counts <- read.csv("neil/deseq2_all_samples/norm_counts.csv",header=T,row.names=1)
total_reads <- colSums(norm_counts)
norm_counts <- norm_counts/total_reads*1000000
norm_counts <- log2(norm_counts)

upreg <- subset(info_data, baseMean > 100 & log2FoldChange > 1 & padj < 0.01, select = gene_symbol)
...
rownames(up_norm) <- upreg$gene

downreg <- subset(info_data, baseMean > 100 & log2FoldChange < -1 & padj < 0.01, select = gene_symbol)
...
rownames(down_norm) <- downreg$gene
